...
namespace sv
{
    ...
    SphericalVoronoiCore::SphericalVoronoiCore()
            : scanLine(), nbSteps(0), debugMode(false)
    {

    }

    SphericalVoronoiCore::SphericalVoronoiCore(const std::vector<IndexedDirection>& directions)
    : scanLine(), nbSteps(0), debugMode(false)
    {
        this->setDirections(directions);
    }

    void SphericalVoronoiCore::setDirections(const std::vector<IndexedDirection>& directions)
    {
        using namespace glm;
        using namespace std;

        //std::vector<std::pair<int, Real3>> sortedDirections;
        std::vector<std::tuple<int, Real3, int>> sortedDirections;
        for (auto& dir : directions)
        {
            //sortedDirections.push_back(std::pair<int, Real3>(sortedDirections.size(), dir.direction));
            sortedDirections.push_back(std::tuple<int, Real3, int>(sortedDirections.size(), dir.direction, dir.index));
        ...
        std::sort(sortedDirections.begin(), sortedDirections.end(),
                  [](const std::tuple<int, Real3, int>& a, const std::tuple<int, Real3, int>& b) {
        ...
        for (size_t i=0; i<sortedDirections.size(); ++i)
        {
            ...
            auto& d = std::get<1>(sortedDirections[i]);
            ...
            auto c = shared_ptr<cell>(new cell(std::get<0>(sortedDirections[i]), p, std::get<2>(sortedDirections[i])));
            //std::cout << "push back cell " << i << std::endl;
            cells.push_back(c);
    ...
    void SphericalVoronoiCore::step(Real maxDeltaXi)
    {
        ...
        if (!isFinished())
        {
            ...
            SV_DEBUG(std::cout << "step " << nbSteps << " " << scanLine.xi);
            ...
            if (siteEventQueue.size() > 0 && (circleEventQueue.size() == 0 || siteEventQueue[0].theta < circleEventQueue[0]->theta))
            {
                ...
                {
                    scanLine.xi = nextXi;
                    SV_DEBUG(cout << "scanLine.xi -> " << scanLine.xi << endl);
                }
            }
    ...
    std::vector<cell_ptr> SphericalVoronoiCore::correctEdges()
    {

        std::vector<cell_ptr> new_cells;
        new_cells.reserve(cells.size()); // Reserve memory to avoid reallocations
        for (const auto& cell : cells) {
            new_cells.push_back(cloneCell(cell));
        }

        std::cout << "Correct edges" << std::endl;

        if(!isFinished())
        {
            std::cout << "Voronoi Diagram generation not completed" << std::endl;
            return new_cells;
        }
        ...
        {
            bool startHasNegativeZ = false;
            half_edge_ptr he;
        };

        for(sv::cell_ptr c : new_cells)
        {
            // Save the current reference of the half-edge to iterate, 
            // and go along al the edges of the cell. Store the half-edges that
            // intersects with z=0 in a vector

            sv::half_edge_ptr current_he = c->halfEdges[0];
            sv::half_edge_ptr starting = current_he;
            std::vector<IntersectingEdge> list_to_correct;
            IntersectingEdge to_correct_1;
            std::vector<half_edge_ptr> to_correct;

            // structures to sture the new points
            std::tuple<double, double, double> new_start;
            std::tuple<double, double, double> new_end;

            sv::half_edge_ptr he_start_zero;
            sv::half_edge_ptr he_end_zero;

            int he_index = 0;

            do
            {
                auto [intersects, startHasNegativeZ] = intersectsZPlane(current_he);
                if (intersects) {
//                    std::cout << "intersects is true, he is " << current_he << std::endl;
//                    std::cout << "original start point was " << current_he->start->point.position.x << ", "
//                              << current_he->start->point.position.y << ", "
//                              << current_he->start->point.position.z << "]" << std::endl;
//                    std::cout << "original end point was " << current_he->end->point.position.x << ", "
//                              << current_he->end->point.position.y << ", "
//                              << current_he->end->point.position.z << "]" << std::endl;
                    if (!startHasNegativeZ) {
                        //std::cout << "end point" << std::endl;

                        auto _end = findZPlaneIntersection(current_he);
                        new_end = _end.value();
                        current_he->end->point.position.x = std::get<0>(new_end);
                        current_he->end->point.position.y = std::get<1>(new_end);
                        current_he->end->point.position.z = std::get<2>(new_end);
                        he_end_zero = current_he;
                    }
                    else
                    {
                        std::cout << "start point" << std::endl;

                        auto _start = findZPlaneIntersection(current_he);
                        new_start = _start.value();
                        current_he->start->point.position.x = std::get<0>(new_start);
                        current_he->start->point.position.y = std::get<1>(new_start);
                        current_he->start->point.position.z = std::get<2>(new_start);
                        he_start_zero = current_he;
                    }
                ...
                // not more than 2 half-edge can cross the z plane
                if (he_index == 2) {
//                    std::cout << "new point p1 is [" << std::get<0>(new_start) << ", "
//                              << std::get<1>(new_start) << ", "
//                              << std::get<2>(new_start) << "]" << std::endl;
//
//                    std::cout << "new point p2 is [" << std::get<0>(new_end) << ", "
//                              << std::get<1>(new_end) << ", "
//                              << std::get<2>(new_end) << "]" << std::endl;

                    Point p1(std::get<0>(new_start), std::get<1>(new_start), std::get<2>(new_start));
                    Point p2(std::get<0>(new_end), std::get<1>(new_end), std::get<2>(new_end));

                    // Create a new half-edge that is contigous to the z plane, and link the new two he
                    // that intetersect the z plane to it
                    std::shared_ptr<vertex> v1 = std::make_shared<vertex>(p1);
                    std::shared_ptr<vertex> v2 = std::make_shared<vertex>(p2);
                    std::shared_ptr<half_edge> contiguous_to_z = std::make_shared<half_edge>(v1, v2);
                    contiguous_to_z->next = he_start_zero;
                    contiguous_to_z->prev = he_end_zero;
                    he_end_zero->next = contiguous_to_z;

                    break;

                }

                // go to check the next half-edge
                current_he = current_he->next;
            } while(current_he->next != nullptr && starting != current_he);

        }

        std::cout << "result cell size: " << new_cells.size() << std::endl;
        return new_cells;
    ...
    {
        Real theta1 = arc1._cell->point.theta;
        Real phi1 = arc1._cell->point.phi;

        Real theta2 = arc2._cell->point.theta;
        Real phi2 = arc2._cell->point.phi;
...
//            assert(glm::distance(ptA_1.position, ptA_2.position) < eps);
    ...
    void SphericalVoronoiCore::handleSiteEvent(site_event& event)
    {
        ...
        if (beach.size() == 0)
        {
            beach.emplace_back(make_shared<beach_arc>(event._cell));
        }
        else if (beach.size() == 1)
        {
            ...
            auto arc = beach[0];
            auto newArc = beach[1];
            Point p = phiToPoint(event.phi, scanLine.xi, arc->_cell->point.theta, arc->_cell->point.phi);
            std::shared_ptr<vertex> newVertex = std::shared_ptr<vertex>(new vertex(p, event._cell, arc->_cell));
        ...
        else
        {
            ...
            for (beach_type::const_iterator itArc=beach.begin(); itArc!=beach.end(); ++itArc)
            {
                ...
                Real phi_start = arc->_cell->point.phi - M_PI;
                if (intPrev)
                {
                    phi_start = pointPrev.phi;
                }

                Real phi_end = arc->_cell->point.phi + M_PI;
                if (intNext)
                {
                ...
                if (intersectFound)
                {
                    ...
                    beach_type::const_iterator itArc2 = beach.insert(itArc, make_shared<beach_arc>(arc->_cell));
                    itArc = std::next(itArc2);
                    auto arc2 = *itArc2;
                    beach_type::const_iterator itNewArc = beach.insert(itArc, make_shared<beach_arc>(event._cell));
...
//        assert(isArcOnBeach(arc_k));
        ...
        auto newVertex = make_shared<vertex>(event->circle_center, arc_i->_cell, arc_j->_cell, arc_k->_cell);
        vertices.push_back(newVertex);
...
//            assert(itArcK != beach.end());
        ...
        else
        {
            ...
            if (arc_1->_cell->index != arc_i->_cell->index && arc_i->_cell->index != arc_k->_cell->index && arc_1->_cell->index != arc_k->_cell->index)
            {
...
//                assert(isArcOnBeach(arc_k));
            ...
            if (arc_i->_cell->index != arc_k->_cell->index && arc_k->_cell->index != arc_2->_cell->index && arc_i->_cell->index != arc_2->_cell->index)
            {
//                assert(isArcOnBeach(arc_i));
//                assert(isArcOnBeach(arc_k));
//                assert(isArcOnBeach(arc_2));
    ...
    std::tuple<bool, bool> SphericalVoronoiCore::intersectsZPlane(const sv::half_edge_ptr& edge)
    {
        double z1 = edge->start->point.position.z;
        double z2 = edge->end->point.position.z;

        bool intersects = false;
        bool startHasNegativeZ = false;

        // Check if one point is above the plane and the other is below the plane
        if ((z1 > 0 && z2 < 0) || (z1 < 0 && z2 > 0)) {
            intersects = true;
        }

        // Check if either point lies exactly on the z=0 plane
        //if (z1 == 0 || z2 == 0) {
        //    intersects = true;
        //}

        // Determine which point has the negative z value
        if (z2 > 0) {
            startHasNegativeZ = true;
        }

        return std::make_tuple(intersects, startHasNegativeZ);
    }


    std::optional<std::tuple<double, double, double>> SphericalVoronoiCore::findZPlaneIntersection(const sv::half_edge_ptr& edge)
    {
        double x1 = edge->start->point.position.x;
        double y1 = edge->start->point.position.y;
        double z1 = edge->start->point.position.z;
        double x2 = edge->end->point.position.x;
        double y2 = edge->end->point.position.y;
        double z2 = edge->end->point.position.z;

        if ((z1 > 0 && z2 < 0) || (z1 < 0 && z2 > 0)) {
            // Calculate the parameter t where the line intersects z = 0
            double t = z1 / (z1 - z2);
            ...
            std::cout << "findZPlaneIntersection result: " << x << ", " << y << ", " << z << std::endl;
        ...
        // Check if either point lies exactly on the z=0 plane
        if (z1 == 0) {
            return std::make_tuple(edge->start->point.position.x, edge->start->point.position.y, edge->start->point.position.z);
        }

        if (z2 == 0) {
            return std::make_tuple(edge->end->point.position.x, edge->end->point.position.y, edge->end->point.position.z);
        }
